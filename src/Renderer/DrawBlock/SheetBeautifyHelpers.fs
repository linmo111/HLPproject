...
let readRetracedWire(sheet:SheetT.Model)=
    ...
    // let produceResult (retraceList,startpos,reTraceTooFarList, lastIndexRetraced) segment =
    //     let newRetraceList, newRetracedIndex=
    //         if (( segment.IsZero) && (segment.Index<>0)) 
    //         then 
    //             if ((segment.Index <> segments.Length-1) && (differentSigns segments[segment.Index-1].Length segments[segment.Index+1].Length ) ) or (segment.Index == segments.Length-1)then
    //                 segment::retraceList, segment.Index
    //             else retraceList, lastIndexRetraced
                    
    //         else retraceList,lastIndexRetraced
    //     let newReTraceTooFarList= 
    //         if  (segment.Index == lastIndexRetraced+2) then 
    //             if (not segment.IsZero ) && (overlap2D symbolXYPosCords (startPos,startPos) ) 
    //                 then segment::reTraceTooFarList
    //                 else reTraceTooFarList
    //         else reTraceTooFarList

    //     let endpos= calcEndPos startpos asegments[segment.Index] 
